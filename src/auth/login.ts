import { RicardoAuthError } from "../core/errors";
...
/** Auth0 / OAuth parameters. Defaults mirror the captured ricardo Android client. */
export interface OAuthConfig {
  authBaseURL: string; // https://auth.ricardo.ch
  clientId: string;
  redirectUri: string;
  audience: string;
  scope: string;
...
export const DEFAULT_OAUTH: OAuthConfig = {
  authBaseURL: "https://auth.ricardo.ch",
 ...
 * ricardo session via /account/authenticateJWT).
 */
export async function runLoginFlow(opts: {
  ...
  let res = await fetch(authorizeUrl.toString(), {
    method: "GET",
    redirect: "manual",
    headers: htmlHeaders({ Referer: "android-app://ch.ricardo/" }),
  });
  ...
  jar.storeFromResponse(res);
  const callback = new URL(locationOf(res)); // ch.ricardo://...callback?code=...&state=...
  ...
  if (callback.searchParams.get("state") !== state) {
    throw new RicardoAuthError("OAuth state mismatch (possible CSRF)");
  }
